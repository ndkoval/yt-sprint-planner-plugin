 ...
 * stays green. It is exercised for real via `npm run test:integration`, which
...
import { describeIntegration, makeClient, makeRunId, field, readEnv, type RestClient } from './helpers';

describeIntegration('YouTrack integration', () => {
  const env = readEnv();
