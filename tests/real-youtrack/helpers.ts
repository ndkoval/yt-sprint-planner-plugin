/**
 * Shared helpers for the real-YouTrack integration tests (§25).
 *
 ...
 *
 * Every spec must call `describeReal(...)` (or guard with `hasRealInstance`) so the
 * suite self-skips when YT_TEST_BASE_URL is unset.
 */
import { describe } from 'vitest';

export const hasRealInstance = Boolean(process.env.YT_TEST_BASE_URL);
...
export function readRealEnv(): RealEnv {
  return {
...
/** describe() that skips the whole suite unless a real instance is configured. */
export const describeReal: typeof describe.skip = hasRealInstance ? describe : describe.skip;
