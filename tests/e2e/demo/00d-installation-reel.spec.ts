...
test.describe('Marketing reel — installation', () => {
  test('install the app from one ZIP and open its settings', async ({ page }, info) => {
    ...
    await page.goto(
      '/install?reelIntro=1&reelTitle=' +
        encodeURIComponent('Install in one click') +
        '&reelSubtitle=' +
        encodeURIComponent('A single ZIP — no external services'),
    ...
    await cap.say('Open Sprint Capacity Settings to finish.');
    ...
    await cap.say('Pick your board and effort fields — ready to plan.');
