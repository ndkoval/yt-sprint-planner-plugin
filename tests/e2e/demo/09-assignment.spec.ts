...
test.describe('Marketing reel — per-person planning', () => {
  test('assigned load per person + unassigned bucket, updating automatically', async ({
    ...
    await openTab(page, 'manager', 'sprint-2', {
      title: 'Plan per person',
      subtitle: 'Balance the team, keep work owned by direction',
    });
    await cap.say('Assign tasks to people while planning.');
    await closeTitleCard(page);
    await cap.say('And leave some work unassigned on purpose.');
    ...
    await cap.say('The Load bar compares committed work to their capacity.');
    ...
    await cap.say('Unassigned work stays owned by project direction.');
    ...
    await cap.say('The what-fits banner shows if the plan fits.');
    ...
    await cap.say('Plan more work — assign a task to Charlie.');
    ...
    await cap.say('Everything recalculates automatically.');
