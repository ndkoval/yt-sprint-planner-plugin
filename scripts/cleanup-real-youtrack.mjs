/**
 * cleanup:real-youtrack — tear down everything the harness created.
 *
...
runMain('cleanup:real-youtrack', async (log) => {
  const env = readEnv();
  ...
  if (baseUrl && env.adminToken && env.allowDestructive) {
    try {
      ...
      // Custom fields are global/shared; we detach by deleting the project only.
      // SPIKE: if the pinned build leaves orphan field prototypes, delete them via
      // /api/admin/customFieldSettings/customFields/{id} — left in place by default
      // to avoid removing fields another run may share.
    } catch (err) {
