/**
 * test:integration — orchestrate the YouTrack integration run.
 *
 * Order: require env -> provision (if needed) -> seed -> run integration tests under
 * tests/youtrack -> ALWAYS cleanup (finally).
 *
...
function runVitest(log) {
  log.step('run vitest tests/youtrack');
  const res = spawnSync(
    process.execPath,
    [fromRoot('node_modules', '.bin', 'vitest'), 'run', 'tests/youtrack'],
    { stdio: 'inherit', cwd: REPO_ROOT },
  );
  return res.status ?? 1;
}

runMain('test:integration', async (log) => {
  const env = readEnv();
  if (!env.allowDestructive) {
    throw new Error('YT_TEST_ALLOW_DESTRUCTIVE=true is required for the integration run');
  }
  ...
  try {
    if (!externallyProvided) {
      const code = run(log, 'provision-youtrack.mjs');
    ...
    const seedCode = run(log, 'seed-youtrack.mjs');
    ...
    const cleanupCode = run(log, 'cleanup-youtrack.mjs');
