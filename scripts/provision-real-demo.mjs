/**
 * provision-real-demo — make the running real YouTrack fully demo-ready in one shot:
 * build+pack the app, (clean) install it, seed a project/team/managed-sprint/issues,
 * and attach the app to the project. Assumes a real YouTrack is already running at
...
function run(log, label, cmd, args) {
  log.step(label);
...
runMain('provision-real-demo', async (log) => {
  run(log, 'build', 'npm', ['run', 'build']);
  run(log, 'pack', 'npm', ['run', 'pack']);
  run(log, 'install app (clean)', 'node', ['scripts/install-app-real-youtrack.mjs']);
  run(log, 'seed project + team + sprint', 'node', ['scripts/setup-real-youtrack-demo.mjs']);
  run(log, 'attach app to project', 'node', ['scripts/attach-app-to-project.mjs']);
  log.info('real YouTrack is demo-ready');
});
